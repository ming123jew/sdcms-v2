    ...
    public function get_category_for_category_list($context)
    {
        ...
        if($all)
        {
            ...
            foreach ($all as $n=> $r)
            {
                ...
                $all[$n]['str_manage'] = (check_role('Admin','Category','category_edit',$context)) ?'<a href="'.url('','','category_edit',["id" => $r['id']]).'">编辑</a> |':'';
                $all[$n]['str_manage'] .= (check_role('Admin','Category','category_delete',$context)) ?'<a  onclick="category_delete('.$r['id'].')" href="javascript:;">删除</a>':'';
